...
// Position pools for spreading icons around viewport - small left buffer to prevent cutoff
const POSITION_POOL = [
  { x: '20%', y: '35%', size: 100 },
  { x: '85%', y: '40%', size: 90 },
  { x: '35%', y: '70%', size: 110 },
...
export default function InteractiveSpeakerNav({
  ...
  const [decorativeNodes, setDecorativeNodes] = useState<Array<{ x: string; y: string; size: number; svg: string; delay: number }>>([])
  ...
  // Generate decorative floating nodes
  useEffect(() => {
    const nodes = Array.from({ length: 20 }, (_, i) => ({
      x: `${Math.random() * 100}%`,
      y: `${Math.random() * 100}%`,
      size: 15 + Math.random() * 15, // 15-30px
      svg: generateRandomIcon(15 + Math.random() * 15),
      delay: Math.random() * 2
    }))
